                                    ...
                                    <tr>
                                        ...
                                        <td>
                                                @if(count($data->product_variations) > 0 )
                                                    <select name="variation[]" onchange="setFilterType(this)" class="form-control" data-product-id = "{{ $data->id }}" onchange()
                                                        id="variation">
                                                        <option value="">Select Variation</option>
                                                        @foreach ($data->product_variations as $variation)
                                                            @if($data->id == $variation->product_id)
                                                            <option data-variation_id = {{ $variation->id }} value="{{ $variation->size?->size }}-{{ $variation->color?->color }}" >{{ $variation->size?->size }}-{{ $variation->color?->color }}
                                                            </option>
                                                            @endif
                                                        @endforeach
                                                    </select>
                                        ...
                                        <td>
                                            <a href="#" class="btn btn-primary-rgba generated_barcode" id="barcod"
                                                data-name="{{ $data->name }}" data-code="{{ $data->barcode }}"
                                                data-size='' data-id="{{ $data->id }}"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" id="modal_cancel" >Cancel</button>
...
@push('js')
    <script>
        ...
        function setFilterType($this) {
            ...
            // console.log("variaiton: " +filterType)
        }
        $(document).on('click', '#modal_cancel', function() {
            $("#bar_code_modal").modal('hide');
            let ad = $('.generated_barcode').attr('data-size', '');
        });
        $(document).on('click', '.generated_barcode', function() {
            ...
            var variation_id = $('#variation option:selected').attr('data-variation_id');
            var full_code = code; //+ "" + variation_id
            ...
            let url = "{{ route('product-barcode', 'value') }}".replace('value', full_code);
                        ...
                        <table class="table table-bordered">`;
                            for ($i = 0; $i < 10; $i++) {
                                ...
                                for ($j = 0; $j < 3; $j++) {
                                    barcode +=
                                    `<td>
                                        <p style="margin-bottom:2px; line-height:9px; margin-top:5px; font-size: 11px; color:black;"><strong>${company}</strong></p>
                                        ${data}
                                        <p style="margin-bottom:2px; line-height:9px; margin-top:3px; font-size: 9px; color:black;"><strong>${name}</strong></p>
                                        <p style="margin-bottom:2px; line-height:9px; margin-top:3px; font-size: 9px; color:black;"><strong>${variation}</strong></p>
                    ...
                    barcode += `<div class="text-center p-4 print_area" id="barcode" style="padding:0 !important">
                        <table class="table" style="margin-bottom:0 !important;">
                            <tr>
                                ...
                                <p style="margin-bottom:0px; line-height:9px; margin-top:2px; font-size: 10px; color:black;"><strong>${full_code.split('').join(' ')}</strong></p>

                                <p style="margin-bottom:2px; line-height:9px; margin-top:0px; font-size: 9px; color:black;"><strong>${name}</strong></p>
                                <p style="margin-bottom:2px; line-height:9px; margin-top:-2px; font-size: 9px; color:black;"><strong>${variation}</strong></p>
        ...
        // $('#variation').on('change',()=>{
        //     var selectedOption = $('#variation option:selected').val();
        // });
