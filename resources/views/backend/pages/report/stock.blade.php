...
@section('action-button')
    ...
    <a href="{{route('report.stock.pdf')}}" class="btn add_list_btn btn-sm">pdf</a>
@endsection
@push('css')
<style>
    @media print{
                        ...
                        <table class="table table-striped text-center">
                            <thead class="header_bg">
                                <tr>
                                    ...
                                    <th class="header_style_right">Available Price</th>
                                </tr>
                            </thead>
                            ...
                            <tbody>
                                @forelse($products as $data)
                                    @php
                                        ...
                                        $stock = (float)$stock_qty * $data->purchase_price;
                                    @endphp
                                    <tr>
                                        ...
                                        <td>{{ $damaged_qty }}</td>
                                        <td>{{ $stock_qty }}</td>
                                        <td class="table_data_style_right">{{ $stock }} {{ empty(get_setting('com_currency')) ? : get_setting('com_currency') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                            ...
                            <tfooter>
                                <tr class="header_bg">
                                    <td colspan="8" class="header_style_left text-white"><strong>Total Available Stock Price: </strong></td>
                                    <td colspan="2" class="header_style_right text-white"><strong>{{number_format($totalPrice,2)}} {{ empty(get_setting('com_currency')) ? : get_setting('com_currency') }}</strong></td>
                                </tr>
                            </tfooter>
                        </table>
                    </div>
