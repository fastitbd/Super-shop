...
@section('section-title', 'Sub Category')
...
@section('content')
                            ...
                            <tbody>
                                @forelse($subcategories as $data)
                                        ...
                                        <td class="table_data_style_right">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    @if (check_permission('subCategory.update'))
                                                    ...
                                                    {{-- delete --}}
                                                    @if (check_permission('subCategory.destroy'))
                                                        @if ($count_cat < 1)
                                    ...
                                    <form action="{{ route('subCategory.update', $data->id) }}" method="POST"
                                        ...
                                        <x-input label="Sub Category Name " type="text" name="name" placeholder="Enter Category Name" value="{{ $data->name }}" required />
                                        ...
                                        <x-input label="Category Order " type="number" name="order_by" placeholder="Enter Category order" value="{{ $data->order_by }}" />
                                        </x-edit-modal>
                                    </form>
    ...
    <form action="{{ route('subCategory.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <x-add-modal title="Add Sub Category" sizeClass="modal-md">
            <x-input label="Sub Category Name " type="text" name="name" placeholder="Enter Category Name" required />
            <x-select label="Category" name="category_id">
